...
namespace ERAAuthentication.SRP6
{
    internal partial class HandShake
    {
        ...
        {
            if (HandShake.LogonManager == null)
                throw new HandShakeException("No HandShake.passive functions are available until LogonManager is provided.");

            if (this.State != HandShakeState.NotInitialized && HandShakeState.AllowResponse.HasFlag(this.State) == false)
                throw new HandShakeException("Double Request");
        ...
        private void VerificationOfActiveParty(SRPVerification verification)
        {
            ...
            Logger.Verbose("It was Bob whom received SRPVerification");

            // Hello I am the one that is being connected to. So let's generate 
            // the value M I should have in the SRPPackedData Object.
            Byte[] M = SRPFunctions.CalcM(N, g, _request.Username.ToLower(), _response.Salt, _request.A, _cache.B, _cache.K);
